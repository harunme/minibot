...
from nanobot.agent.tools.music import PlayMusicTool
...
class AgentLoop:
    ...
    def __init__(
        ...
        self._vectorstore_config = vectorstore_config
    ...
    def _register_default_tools(self) -> None:
        ...
        play_music_tool = PlayMusicTool(message_tool=message_tool)
        self.tools.register(play_music_tool)
        self.tools.register(SpawnTool(manager=self.subagents))
        if self.cron_service:
            self.tools.register(
                CronTool(self.cron_service, default_timezone=self.context.timezone or "UTC")
            )
        if self._vectorstore_config and self._vectorstore_config.provider != "none":
            from nanobot.agent.tools.knowledge import KnowledgeSearchTool

            self.tools.register(KnowledgeSearchTool(vectorstore_config=self._vectorstore_config))
    ...
    def _set_tool_context(
        ...
        import inspect
        for name in ("message", "spawn", "cron", "play_music"):
            if tool := self.tools.get(name):
                if hasattr(tool, "set_context"):
                    sig = inspect.signature(tool.set_context)
                    if "message_id" in sig.parameters:
                        tool.set_context(channel, chat_id, message_id=message_id)
                    else:
